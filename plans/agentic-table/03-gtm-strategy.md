...
| **招聘 / 人才 sourcing** | ✅ | 高（错招成本 = 数月薪资） | 分散，无主导 | **强**（见下） | ✅ **推荐** |
| 采购 / 供应商风险 | ✅ | 高 | 分散 | 无 | ✅ 次选 |
...
- **⚠️ 假设待确认**：本仓库所在的工作区已接入 LinkedIn（人物搜索 / profile / 私信 / 邀请管理）、
  Gmail（搜索 / 读取 / 发送 / 草稿）、以及带向量检索的 context card 存储和 workflow 运行时。
  如果这些是自有产品资产，那么招聘垂直的**连接器与动作出口几乎已经建好了**，
  这会把 MVP 的工程量砍掉一半以上。**这一条需要你确认后再定稿。**
