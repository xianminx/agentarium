...
def test_run_action_creates_task_and_triggers(api_client, user, monkeypatch):
    ...
    # Mock the OpenAI function to avoid actual API calls
    def fake_openai(agent_obj, prompt, max_tokens=1024):
        return "Mocked response"

    monkeypatch.setattr("apps.tasks.tasks.run_agent_sync", fake_openai)
    ...
    resp = api_client.post(
        "/api/tasks/run/", {"agent": agent.id, "input_text": "Hello world"}
    )
...
def test_task_list_filters(api_client, user):
    ...
    AgentTask.objects.create(
        agent=agent, owner=user, input_text="1", status="completed"
    )
