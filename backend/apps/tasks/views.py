...
class TaskViewSet(viewsets.ModelViewSet):
    ...
    def run(self, request):
        ...
        if not agent_id or not input_text:
            return Response({"detail": "agent and input_text are required"}, status=status.HTTP_400_BAD_REQUEST)
        try:
            agent = Agent.objects.get(pk=agent_id, owner=request.user)
        except Agent.DoesNotExist:
            return Response({"detail": "Agent not found"}, status=status.HTTP_404_NOT_FOUND)

        task = AgentTask.objects.create(agent=agent, owner=request.user, input_text=input_text, status=AgentTask.STATUS_PENDING)
